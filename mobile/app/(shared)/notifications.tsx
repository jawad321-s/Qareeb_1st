...
const notifsFor = (tint: string): Record<'en' | 'ar', Notif[]> => ({
  en: [
    { id: 'n1', icon: 'wallet', color: tint, title: 'New offer received', body: 'Omar Khalid offered 120 ₪ for your plumbing request.', ts: Date.now() - 12 * 60_000, read: false, route: '/(shared)/request/req_1' },
    ...
    { id: 'n3', icon: 'message', color: tint, title: 'New message', body: 'Tariq: Please keep the AC off until I arrive.', ts: Date.now() - 3 * 3600_000, read: true, route: '/(shared)/chat/req_2' },
  ...
  ar: [
    { id: 'n1', icon: 'wallet', color: tint, title: 'عرض جديد', body: 'قدّم عمر خالد عرضاً بـ 120 ₪ لطلب السباكة.', ts: Date.now() - 12 * 60_000, read: false, route: '/(shared)/request/req_1' },
    ...
    { id: 'n3', icon: 'message', color: tint, title: 'رسالة جديدة', body: 'طارق: من فضلك أبقِ التكييف مطفأً حتى أصل.', ts: Date.now() - 3 * 3600_000, read: true, route: '/(shared)/chat/req_2' },
...
export default function Notifications() {
  ...
  const { t, locale } = useT();
  const notifs = notifsFor(colors.tint)[locale];
